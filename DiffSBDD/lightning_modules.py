...
class LigandPocketDDPM(pl.LightningModule):
    ...
    def generate_ligands(self, pdb_file, n_samples, pocket_ids=None,
        ...
        # Load PDB
        # I/O compatibility: recent PDB entries are distributed as mmCIF only.
        # Parser choice does not affect pocket construction or sampling.
        if str(pdb_file).endswith('.cif'):
            from Bio.PDB.MMCIFParser import MMCIFParser
            pdb_struct = MMCIFParser(QUIET=True).get_structure('', pdb_file)[0]
        else:
