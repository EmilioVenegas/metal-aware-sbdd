"""Overbuild strictly clean external Zinc benchmark with >= 35 independent clusters.
...
3. Genuine Drug-like Native Inhibitor: >= 8 heavy atoms, non-solvent/non-amino-acid, with a donor atom (N, O, S, P, Cl, F) within <= 2.5 A of catalytic Zn.
4. Independent sequence clusters: < 30% pairwise sequence identity between distinct target families/clusters.
5. Overbuild target: >= 35 distinct 30%-identity clusters.
"""
...
COMMON_SOLVENTS = {
    ...
    'IOD', 'BR', 'K', 'CS', 'LI', 'RB', 'SR', 'BA', 'IMD', 'MLI', 'GSU', 'TAR',
    'ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 'HIS', 'ILE',
    'LEU', 'LYS', 'MET', 'PHE', 'PRO', 'SER', 'THR', 'TRP', 'TYR', 'VAL',
    'KCX', 'MHS', 'MSE', 'CSE', 'CSO', 'CME', 'CAS', 'OCS', 'SEP', 'TPO', 'PTR'
}
...
def query_rcsb_zn_ligand_structures(max_results=5000):
    query = {
        "query": {
            "type": "group",
...
def inspect_pdb_worker(pdb_id):
    ...
    for zn in zn_atoms:
        ...
        for lig_key, lig_atom_list in ligands.items():
            if len(lig_atom_list) < 8: # genuine drug-like inhibitor (>= 8 heavy atoms)
                continue
...
def main():
    ...
    print("Fetching candidate Zn-ligand structures from RCSB (5000 entries)...")
    rcsb_candidates = query_rcsb_zn_ligand_structures(max_results=5000)
    ...
    print(f"\nTotal targets passing catalytic Zn, <=2.5A drug-like inhibitor coordination, and 0 CrossDocked train overlap: {len(passed_targets)}")
    sys.stdout.flush()
    ...
    for idx, c in enumerate(clusters_30):
        ...
        print(f"Cluster {idx+1:2d} (rep {rep['pdb_id']}, {len(c)} members, lig={rep['ligand_resname']} [{rep['ligand_num_heavy_atoms']} HA], d={rep['min_zn_ligand_dist']:.2f}A, coord={rep['coordinating_ligand_atoms'][:2]}): {', '.join(members[:4])}")
    ...
    payload = {
        ...
        "provenance": {
            ...
            "criteria": [
                ...
                "Directly coordinated native drug-like inhibitor (>=8 heavy atoms, non-amino-acid) with donor (N/O/S/P/Cl/F) within <= 2.5 A of catalytic Zn",
