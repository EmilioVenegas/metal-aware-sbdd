...
def check_molecule(coords: np.ndarray, elements: list[str], metal_xyz: np.ndarray,
    ...
    # Coordination geometry over the ligand-contributed valid donors.
    cn = len(valid_coord)
    rms_dev = None
    if cn >= 2 and cn in IDEAL_ANGLES:
    ...
    return {
        ...
        "has_valid_coordination": cn > 0,
        "n_valid_coordination": cn,
    ...
    for k, mol in enumerate(Chem.SDMolSupplier(str(sdf), sanitize=False, removeHs=False)):
        ...
        if len(pos) == 0:
            out.append({"pdb_id": pdb_id, "source": source, "mol_index": k,
                        "unreadable": True})
            continue
        rec = check_molecule(pos, els, metal_xyz, metal)
