...
def main():
    ...
    if args.figure in ("all", "3"):
        fig3_valid_coordination.make_figure(outdir, arm_a, arm_b, native, xray, pdb_to_cluster)
    if args.figure in ("all", "4"):
        fig4_distance_distribution.make_figure(outdir, arm_a, arm_b, native, xray)
    if args.figure in ("all", "5"):
        fig5_coordination_spheres.make_figure(outdir, arm_a, arm_b, native, xray)
